(function () {
  ...
  function makeApp(desc) {
    ...
    var algos = [["morse", "Morse Code"], ["spelling", "Spelling Alphabet"], ["affine", "Affine"], ["rot", "ROT"], ["vigenere", "Vigenere"], ["bacon", "Bacon"], ["alpha_sub", "Alphabetical Substitution"], ["railfence", "Railfence"], ["base32", "Base32"], ["base64", "Base64"], ["ascii85", "Ascii85"], ["unicode", "Unicode Notation"], ["integer", "Integer"]];
    if (isCrack) algos = [["all", "All"]].concat(algos);
    ...
    return { id: desc.id, title: desc.title, glyph: desc.glyph || (isCrack ? "K" : "C"), kind: "script", width: 940, height: 620, menu: "Hacking Tools", external: true, showInMenu: true, singleton: true, render: function (body, win) {
      ...
      (isCrack ? [["bruteforce", "Bruteforce"]] : [["encrypt", "Encrypt"], ["decrypt", "Decrypt"]]).forEach(function (m) { var o = document.createElement("option"); o.value = m[0]; o.textContent = m[1]; mode.appendChild(o); });
      ...
      function updateFields() {
        ...
        show(radix.parentNode, a === "integer" || (isCrack && a === "all")); show(preserve.parentNode, ["affine", "vigenere", "alpha_sub"].indexOf(a) >= 0); show(words.parentNode, isCrack && (a === "vigenere" || a === "all"));
      }
      ...
      function runAll() { inputs().then(function (list) { var lines = [], o = opts(); if (!list.length) { setOut("No input."); return; } list.forEach(function (item) { if (list.length > 1) lines.push("[" + item.name + "]"); lines = lines.concat(isCrack ? crackOne(algo.value, item.text, o) : [run(algo.value, mode.value, item.text, o)]); if (list.length > 1) lines.push(""); }); setOut(lines); }); }
      fillVariants(); updateFields(); renderFiles();
      algo.onchange = function () { fillVariants(); updateFields(); }; src.onchange = updateFields; addBtn.onclick = pickFile; remBtn.onclick = function () { if (selected >= 0) { files.splice(selected, 1); selected = Math.min(selected, files.length - 1); renderFiles(); } }; clearFilesBtn.onclick = function () { files = []; selected = -1; renderFiles(); };
